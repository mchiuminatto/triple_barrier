...
class TestPlots:
    ...
    def test_calculate_plotting_range_extension_full_range(self, prepare_price_data):
        ...
        date_from_extended, date_to_extended = plotter._calculate_plotting_range_extension(date_from, date_to)
        
        assert date_from_extended == datetime.strptime("2023-01-02 01:35:00", "%Y-%m-%d %H:%M:%S")
        assert date_to_extended == datetime.strptime("2023-01-02 03:25:00", "%Y-%m-%d %H:%M:%S")
